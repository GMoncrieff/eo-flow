...
class F1Score(InitializableMetric):
    ...
    def init_from_config(self, model_config=None):
        ...
        if model_config is not None and 'n_classes' in model_config:
            self.metric = tfa.metrics.F1Score(num_classes=model_config['n_classes'], average=None)
        else:
            print("n_classes not found in model config or model config not provided. Using default  value.")
            self.metric = tfa.metrics.F1Score(num_classes=self.default_max_classes, average=None)
